...
exports.getSingleEnrolledCourse = async (req, res) => {
  ...
  const course = await SchoolCourses.findOne({ _id: enrolledCourseId, school:req.user._id  })
...
exports.loginFlowSchool = async (req, res) => {
  ...
  res
    ...
    .json({ message: "School Login successful!", token, user: schoolData });
};
...
exports.forgotPassword = async (req, res) => {
  ...
  res
    .status(StatusCodes.OK)
    .json({ message: `Please enter the code sent to ${email}` });
};
...
exports.changePassword = async (req, res) => {
  ...
  if (!validPassword)
    return res.status(400).send("Your old password is incorrect!");
  // hash the password
...
exports.addEmailNotificationadmin = async (req, res) => {
  ...
  const emailExists = school.email_notification.some(
  ...
  res
    .status(StatusCodes.OK)
    .json({ message: "Admin email notification added successfully!" });
};
...
exports.addStudentsToCourseEnrollment = async (req, res) => {
  ...
  res
    .status(StatusCodes.OK)
    .json({ message: "Students invited to course successfully!" });
};
...
exports.removeSchoolAdmin = async (req, res) => {
  ...
  res
    .status(StatusCodes.OK)
    .json({ message: "School admin removed successfully!" });
};
